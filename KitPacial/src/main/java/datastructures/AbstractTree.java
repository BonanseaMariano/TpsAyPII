...
public abstract class AbstractTree<E> implements Tree<E> {
    ...
    public Map<Position<E>, List<Position<E>>> branchMap() {
        Map<Position<E>, List<Position<E>>> snapshot = new ChainHashMap<>();

        for (Position<E> p : externals()) {
            List<Position<E>> lista = new ArrayList<>();
            snapshot.put(p, lista);
    ...
    /**
     * Retorna un Map donde la clave es la profundidad y el valor una lista con
    ...
    public Map<Integer, List<E>> mapDepth() {
        Map<Integer, List<E>> mapdepth = new UnsortedTableMap<>();
        for (Position<E> p : breadthfirst()) {
            if (mapdepth.get(depth(p)) == null) { //La entrada en el mapa todavia no existe
                mapdepth.put(depth(p), new ArrayList<E>());
                mapdepth.get(depth(p)).add((E) p);
            } else {
                mapdepth.get(depth(p)).add((E) p);
            }
        }
