...
public abstract class AbstractTree<E> implements Tree<E> {
    ...
    public Iterable<Position<E>> breadthfirst() {
        ...
        if (!isEmpty()) {
            ...
            while (!fringe.isEmpty()) {
                Position<E> p = fringe.dequeue();     // remove from front of the top of the stack
                snapshot.add(p);                      // report this position
                for (Position<E> c : children(p)) {
                    fringe.enqueue(c);                  // add children to the top of the stack
                }
            }
        }
        return snapshot;
    ...
    public Iterable<Position<E>> breadthfirstStack() {
        List<Position<E>> snapshot = new ArrayList<>();
        if (!isEmpty()) {
            Stack<Position<E>> fringe = new ArrayStack<>();
            fringe.push(root());                 // start with the root
            while (!fringe.isEmpty()) {
                Position<E> p = fringe.pop();     // remove from front of the top of the stack
                snapshot.add(p);                      // report this position
                for (Position<E> c : children(p)) {
                    fringe.push(c);                  // add children to the top of the stack
                }
            }
        }
        return snapshot;
